import type { Translations } from "./en";

export const pt: Translations = {
  nav: {
    work: "Projetos",
    about: "Sobre",
    services: "Serviços",
    stack: "Stack",
    contact: "Entre em contato",
  },
  hero: {
    badge: "Engenharia de produto founder-led · Brasil → mundo",
    title: "SaaS, sistemas e automações com IA",
    titleHighlight: "construídos para operar de verdade.",
    subtitle:
      "Transformo processos manuais e ideias validadas em produto funcionando — interface, backend, integrações e deploy. Você fala direto comigo, acompanha por marcos e valida cada entrega em um preview navegável.",
    subtitleMobile:
      "Do processo manual ao produto em produção, com escopo claro, preview navegável e revisão direta do founder.",
    terminalTitle: "je4ndev/product-engineering",
    terminalCommand: "ship produto --saas --automacao --agentes",
    terminalOutput: "problema -> escopo -> preview -> producao",
    cta: "Ver 3 cases principais",
    secondaryCta: "Descrever meu projeto",
    email: "jean@je4ndev.com",
    whatsapp: "Falar no WhatsApp",
    proof:
      "Estratégia, produto, interface, backend, banco, integrações e operação em uma entrega responsável.",
    stats: [
      { value: "12", label: "cases catalogados" },
      { value: "6", label: "produtos públicos" },
      { value: "Full-stack", label: "entrega ponta a ponta" },
      { value: "PT/EN", label: "atuação global" },
    ],
    strengths: [
      "MVPs SaaS com auth, billing e painel admin",
      "Sistemas internos que substituem planilhas e retrabalho",
      "Automações e integrações conectadas à operação real",
      "Agentes de IA com memória, permissões e revisão humana",
    ],
  },
  work: {
    label: "Nossos produtos · live em produção",
    title: "Construímos, entregamos e mantemos",
    viewCase: "Ver estudo de caso",
    viewProject: "Ver projeto",
    hoverProblem: "Problema",
    hoverDelivery: "Entrega",
    hoverResult: "Resultado",
    hoverCtaLive: "Acessar demo",
    hoverCtaCode: "Ver código",
    hoverCtaLike: "Quero algo parecido",
    likeWhatsappTemplate: "Oi Jean, quero algo parecido com {project}.",
  },
  about: {
    label: "Sobre",
    name: "Jean Carlos Vargas",
    bio: "Desenvolvedor full-stack, fundador da JE4NDEV e criador de produtos próprios. Há mais de 10 anos transformo problemas de operação em software: plataformas SaaS, sistemas internos, automações, integrações e agentes de IA. Baseado em Paranavaí, Paraná, com atuação remota no Brasil e no exterior.",
    experience: "10+ anos",
  },
  stack: {
    label: "Tech Stack",
    title: "Ferramentas que uso",
    categories: {
      frontend: "Frontend",
      backend: "Backend",
      database: "Banco de Dados",
      ai: "IA & Automação",
      devops: "DevOps",
      web3: "Web3 & Smart Contracts",
      tools: "Ferramentas",
    },
  },
  services: {
    label: "Capacidades",
    title: "O que resolvemos",
    items: [
      {
        title: "SaaS e sistemas sob medida",
        description:
          "Produtos com autenticação, permissões, billing, painel admin, multi-tenant e métricas conectadas ao negócio",
      },
      {
        title: "Automações e integrações",
        description:
          "APIs, CRMs, ERPs, marketplaces, pagamentos e WhatsApp conectados para reduzir tarefa manual e retrabalho",
      },
      {
        title: "Agentes de IA na sua infraestrutura",
        description:
          "Agentes com contexto, memória, ferramentas, permissões e aprovação humana rodando na sua VPS ou ambiente privado",
      },
      {
        title: "Plataformas e experiências web",
        description:
          "Sites de produto, portais, dashboards e aplicações responsivas com foco em clareza, conversão e operação",
      },
      {
        title: "Evolução e recuperação técnica",
        description:
          "Diagnóstico, arquitetura, observabilidade, correção de integrações e evolução de produtos que já estão em uso",
      },
      {
        title: "Web3 e Solidity sob demanda",
        description:
          "Smart contracts, plataformas Web3 e revisão de segurança como capacidade especializada quando o produto exige",
      },
    ],
    cta: "Avaliar meu caso",
    whatsappMessage: "Oi Jean, quero avaliar meu caso de {service}.",
  },
  contact: {
    title: "Vamos tirar o gargalo",
    titleHighlight: "da ideia e colocar em produção.",
    subtitle:
      "Me conte o que hoje é manual, lento, desconectado ou difícil de medir. Eu devolvo perguntas objetivas e um próximo passo claro — sem proposta genérica.",
    whatsapp: "Descrever meu projeto",
    trustline:
      "Contato direto com o founder · primeiro marco verificável · escopo e investimento definidos antes de construir",
    founderName: "Jean Carlos Vargas",
    founderRole: "Founder · Product Engineer · Full-stack",
    founderLocation: "Paranavaí · PR · Atendimento remoto no Brasil e exterior",
    terminalTitle: "projeto/intake.md",
    terminalLines: [
      "descreva o gargalo ou oportunidade",
      "mapeamos fluxo, integrações e risco",
      "definimos o primeiro marco verificável",
      "preview navegável antes do próximo marco",
      "deploy e operação conforme o escopo",
    ],
    githubCta: "Ver código e projetos públicos",
  },
  githubProof: {
    label: "Código aberto · histórico verificável",
    title: "Não dependa só da copy deste site",
    subtitle:
      "Use os links públicos para inspecionar como eu penso produto, organizo código e documento sistemas. Quando a prova é privada ou editorial, o case sinaliza isso claramente.",
    bullets: [
      {
        title: "Produtos públicos navegáveis",
        body: "ArchScene, NexPanel, Vultrix 3D e StopUltimate mostram interface, fluxo e proposta em funcionamento.",
      },
      {
        title: "Infraestrutura de agentes open source",
        body: "O Hermes Workspace documenta uma base real para agentes com ferramentas, memória, contexto e operação local.",
      },
      {
        title: "Cases com contexto e limites",
        body: "Cada página separa problema, construção, resultado e tipo de evidência — sem apresentar conceito como produto validado.",
      },
      {
        title: "Capacidade técnica especializada",
        body: "Projetos Web3 e Solidity permanecem disponíveis como prova técnica complementar, sem diluir a oferta principal de produto.",
      },
    ],
    metricsTitle: "Prova rápida",
    metrics: [
      { label: "Cases catalogados", value: "12" },
      { label: "Produtos públicos", value: "6" },
      { label: "Open source", value: "MIT" },
      { label: "Idiomas", value: "PT/EN" },
    ],
    verificationNote:
      "Links, repositórios e páginas de case indicam o nível de prova disponível antes de qualquer proposta.",
    cta: "Abrir GitHub",
  },
  footer: {
    copyright: "(c) 2026 Jean Carlos Vargas",
  },
  testimonials: {
    label: "Prova social",
    title: "Depoimentos verificados",
    items: [],
  },
  process: {
    label: "Como entregamos",
    title: "Um primeiro marco pequeno o bastante para validar. Completo o bastante para gerar valor.",
    subtitle:
      "Cada etapa termina em evidência que você consegue abrir, testar e aprovar. O prazo final nasce do escopo — não de uma promessa genérica.",
    steps: [
      {
        title: "Diagnóstico e critério de sucesso",
        description:
          "Mapeamos o gargalo, os usuários, o fluxo atual, as integrações e o que precisa ser verdade para chamar o primeiro marco de pronto.",
        deliverable: "PROBLEMA → ESCOPO",
      },
      {
        title: "Primeiro marco navegável",
        description:
          "Construímos o menor fluxo ponta a ponta que prova a direção do produto. Você testa em preview e corrige o rumo antes de ampliar.",
        deliverable: "PREVIEW → FEEDBACK",
      },
      {
        title: "QA técnico e de produto",
        description:
          "Testes automatizados onde fazem sentido, revisão manual no browser, segurança proporcional ao risco e registro dos limites conhecidos.",
        deliverable: "GATES → ACEITE",
      },
      {
        title: "Deploy, handoff e próximo marco",
        description:
          "Colocamos em produção quando isso faz parte do escopo, entregamos acessos e documentação e definimos suporte ou evolução sem dependência forçada.",
        deliverable: "PRODUÇÃO → PRÓXIMO MARCO",
      },
    ],
  },
  team: {
    label: "",
    title: "",
    subtitle: "",
  },
  universe: {
    label: "Arquivo de projetos · 12 cases catalogados",
    title: "Mais produtos, sistemas e experimentos",
    subtitle:
      "Os três cases acima são a vitrine principal. Aqui você pode filtrar o restante do repertório por área e abrir a evidência disponível de cada projeto.",
    filters: {
      all: "Tudo",
      saas: "SaaS",
      ai: "IA",
      web3: "Web3 / Security",
      internal: "Ferramentas internas",
      games: "Games / Social",
    },
    searchPlaceholder: "Buscar projeto, tecnologia ou categoria...",
    resultLabel: "projeto encontrado",
    resultsLabel: "projetos encontrados",
    emptyTitle: "Nenhum projeto nesse recorte",
    emptyBody: "Tente outro filtro ou remova parte da busca.",
    proofApproved: "Prova pública",
    proofPrivate: "Demo privada",
    proofPending: "Captura em revisão",
    proofEditorial: "Case editorial",
    showAll: "Ver arquivo completo",
    showLess: "Mostrar menos",
    footer: "Cada card informa o tipo de prova disponível e abre um case com contexto, stack e próximos links.",
  },
  pricing: {
    label: "Entrada comercial",
    title: "Diagnóstico + primeiro marco navegável.",
    subtitle:
      "Você descreve o gargalo. Eu devolvo perguntas objetivas, escopo, critério de sucesso e o menor fluxo ponta a ponta que vale construir primeiro — com prazo, investimento e exclusões definidos antes do build.",
    plans: [
      {
        name: "Diagnóstico + primeiro marco navegável",
        tagline: "Para transformar um gargalo real em um fluxo que você consegue abrir, testar e aprovar",
        price: "Entrada por marcos",
        priceNote: "Escopo, aceite, prazo e investimento definidos antes do build",
        features: [
          "Mapeamento do fluxo, usuários, gargalos e integrações",
          "Critério de sucesso, entregáveis e o que fica fora",
          "Primeiro slice funcional ponta a ponta",
          "Preview navegável, QA técnico e revisão manual no browser",
          "Próximo marco proposto somente depois da evidência",
        ],
        cta: "Descrever meu gargalo",
        featured: true,
        badge: "Entrada recomendada",
      },
      {
        name: "Evolução de produto existente",
        tagline: "Para um sistema em uso que precisa avançar sem acumular caos",
        price: "Sprint ou acompanhamento",
        priceNote: "Formato e capacidade definidos com transparência",
        features: [
          "Diagnóstico técnico e backlog priorizado por impacto e risco",
          "Correção de integrações, gargalos e dívida crítica",
          "Observabilidade, hotfixes e operação quando contratados",
          "Novos fluxos, automações e melhorias de produto",
          "Pausa, renovação, garantia e responsabilidades documentadas",
        ],
        cta: "Avaliar produto atual",
        featured: false,
      },
    ],
    disclaimer:
      "SaaS, automações, agentes privados, plataformas web e Web3 entram como capacidades aplicadas ao problema — não como pacotes genéricos.",
  },
  faq: {
    label: "FAQ",
    title: "Dúvidas antes de começar",
    subtitle: "Respostas diretas sobre processo, escopo, propriedade, segurança e suporte.",
    items: [
      {
        q: "Como isso é diferente de contratar um freela ou uma agência tradicional?",
        a: "A operação é founder-led: você fala diretamente comigo, eu defino a arquitetura e reviso a entrega. Automação e agentes aceleram tarefas, mas escopo, decisões críticas e responsabilidade final continuam sob revisão humana.",
      },
      {
        q: "Como vocês definem prazo e investimento?",
        a: "Primeiro mapeamos fluxo, integrações, riscos e critério de aceite. A proposta fecha o primeiro marco, o prazo, o investimento, as dependências do cliente e o que fica fora. Se ainda houver incerteza demais, começamos por diagnóstico em vez de fingir precisão.",
      },
      {
        q: "Quando eu consigo ver algo funcionando?",
        a: "O plano prioriza um checkpoint navegável cedo, mas a data depende do escopo real. Você recebe um cronograma por marcos depois de validarmos dependências, integrações e riscos técnicos.",
      },
      {
        q: "O código e os acessos ficam comigo?",
        a: "A proposta define propriedade, repositório, contas e handoff. Quando o projeto usa sua infraestrutura, o objetivo é que você mantenha acesso administrativo e não dependa de uma caixa-preta para continuar operando.",
      },
      {
        q: "Vocês assinam NDA e lidam com dados confidenciais?",
        a: "Sim, quando necessário. Também definimos quais dados entram no ambiente, quais provedores são usados e quais acessos cada integração precisa. Nenhum dado do cliente deve virar material público ou treino sem autorização expressa.",
      },
      {
        q: "E se algo falhar depois do deploy?",
        a: "Correções, garantia de defeito e suporte operacional são descritos na proposta conforme o risco e o formato contratado. Monitoramento contínuo, plantão e evolução mensal não são prometidos por padrão: entram quando foram explicitamente contratados.",
      },
      {
        q: "Por que confiar antes de contratar?",
        a: "Abra os produtos públicos, o GitHub e as páginas de case. Cada projeto sinaliza quando a prova é pública, privada ou editorial. A intenção é permitir que você avalie evidência e método antes de aceitar uma proposta.",
      },
    ],
  },
  project: {
    breadcrumbWork: "Projetos",
    demoLabel: "Demo em movimento",
    demoIntro: "Demonstração complementar do fluxo ou resultado do produto.",
    visuals: "Prints do produto",
    visualIntro: "Telas reais, resultados e artefatos revisados do produto.",
    mainVisual: "Print em destaque",
    impact: "Impacto no negócio",
    before: "Antes",
    build: "O que eu construi",
    result: "Resultado",
    resultIntro: "Os principais sinais de resultado do projeto.",
    problem: "O Problema",
    solution: "A Solução",
    visitLive: "Visitar site",
    github: "GitHub",
    docs: "Documentação",
    contact: "Solicitar demonstração",
    proofLabel: "Prova revisada",
    privateProof: "Demonstração privada",
    pendingProof: "Captura do produto em revisão",
    editorialProof: "Apresentação editorial",
    contextLabel: "Contexto do projeto",
    deliveryRecordLabel: "Registro da entrega",
    deliveryRecordIntro: "Papel, arquitetura, estado e limites declarados sem transformar demonstração em promessa.",
    responsibilityLabel: "Meu papel",
    architectureLabel: "Arquitetura",
    currentStateLabel: "Estado atual",
    limitationsLabel: "Limite da prova",
    ctaTitle: "Quer validar um gargalo parecido?",
    ctaSubtitle:
      "Descreva o fluxo que hoje é manual, lento ou difícil de medir. Eu devolvo perguntas objetivas e um primeiro marco verificável — sem compromisso com um projeto infinito.",
    ctaWhatsapp: "Conversar sobre meu projeto",
    ctaEmail: "Enviar email",
    nextProject: "Próximo projeto",
  },
};
